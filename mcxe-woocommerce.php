<?php
	...
	add_action( 'admin_notices', 'J2PME_woocommerce_required_admin_notice' ); } else
	{
		...
		function J2PME_add_mcxe($methods){
			$methods[] = J2PME_Gateway_Mcxe::class;
			return $methods;
		}
        ...
        function J2PME_verify( $request_data ){
            ...
            foreach ($orders as $order) {
				...
				if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
					$order_real = new WC_Order( $order );
					
					$meta = $order_real->get_meta('request_id_gpo', true );
					
					if ($meta == $pedido){
					   $data["status"] = $order_real->get_meta('request_status_gpo', true );
					   return $data;
					}
        	   ...
        	   $order = new WC_Order( $order_id );
        		...
        		if($order)
        		{
					...
					if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
						...
						$order->delete_meta_data('request_id_gpo');
						$order->delete_meta_data('request_status_gpo');

						$order->add_meta_data( 'request_id_gpo', $request_id_gpo );
						$order->add_meta_data( 'request_status_gpo', $estado);
