...
{   
	...
	{
		$this->id                   = "mcxe";
		$this->icon                 = "https://plugins.svn.wordpress.org/wc-ja-ja-pagamentos-multicaixa-express/assets/mcexpress.svg";
	...
	public function process_payment($orderId)
	{
		...
		try {
			...
			$request_data['qrCode'] 	    = $this->get_option( 'qrcode' );
	...
	public function add_checkout_page()
    {
		...
		$page_title = 'Compra Realizada sem Sucesso';
        $nosucess_page_page = get_page_by_title($page_title, 'OBJECT', 'page');
		
        if (empty($nosucess_page_page)) {
            wp_insert_post(
                array(
                'comment_status' => 'close',
                'post_author'    => 1,
                'post_title'     => $page_title,
                'post_name'      => strtolower(str_replace(' ', '_', str_replace(' ', '-', trim($page_title)))),
                'post_status'    => 'publish',
                'post_content'   => 'A sua compra via Multicaixa Expres não foi realizada com sucesso.',
                'post_type'      => 'page'
                )
            );
        }

		$page_title = 'Compra Realizada com Sucesso';
        $sucess_page = get_page_by_title($page_title, 'OBJECT', 'page');

        if (empty($sucess_page)) {
            wp_insert_post(
                array(
                'comment_status' => 'close',
                'post_author'    => 1,
                ...
                'post_name'      => strtolower(str_replace(' ', '_', str_replace(' ', '-', trim($page_title)))),
                'post_status'    => 'publish',
                'post_content'   => 'A sua compra via Multicaixa Expres foi realizada com sucesso.',
                'post_type'      => 'page'
                )
            );
        }
